...
class RoMultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, cos, sin, mask=None):
        B, T_q, D = query.size()
        _, T_k, _ = key.size()
        _, T_v, _ = value.size()

        # Project + reshape to [B, H, T, d_head]
        ...
        V = self.W_v(value).view(B, T_v, self.n_heads, self.d_head).transpose(1, 2)

        # Apply RoPE (expects [B, H, T, d_head])
